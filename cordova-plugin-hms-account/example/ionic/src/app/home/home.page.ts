...
export class HomePage {
  ...
  async accountSignInWithIdToken() {
    ...
    const signInParam: SignInData = {
      ...
      "authParam": AuthParams.DEFAULT_AUTH_REQUEST_PARAM,
      "authScopeList": [AuthScopeList.EMAIL, AuthScopeList.PROFILE],
      "authIdTokenSignAlg": AuthIdTokenSignAlg.PS256
    }
    const packageName = PackageName.ACCOUNT;

    try {
      const res = await this.accountAuthService.signIn(signInParam,packageName);
      console.log(JSON.stringify(res));
  ...
  async accountSilentSignIn() {
    ...
    const packageName = PackageName.ACCOUNT;
    try {
      const res = await this.accountAuthService.silentSignIn(authParam,packageName);
      console.log(JSON.stringify(res));
      alert('silentSignIn -> success :' + JSON.stringify(res));
    } catch (ex) {
      alert('silentSignIn -> Error : ' + JSON.stringify(ex));
  ...
  async containScopes() {
    ...
    try {
      const containScopeRes = await this.authManager.containScopes(authHuaweiId, authScopeList,packageName);
  ...
  async getAuthResult() {
    ...
    try {
      const lastAuthResult = await this.authManager.getAuthResult(packageName);
  ...
  async getAuthResultWithScopes() {
    ...
    try {
      const authResultWithScope = await this.authManager.getAuthResultWithScope(authScopeList,packageName);
  ...
  async addAuthScopes() {
    ...
    try {
      const authScope = await this.authManager.addAuthScopes(8888, authScopeList,packageName);
  ...
  async accountContainScopes() {
    ...
    try {
      const containScopeRes = await this.accountAuthManager.containScopes(authHuaweiId, authScopeList,packageName);
      alert('containScopes -> success: ' + JSON.stringify(containScopeRes));
    } catch (ex) {
      alert('containScopes -> Error : ' + JSON.stringify(ex));
    }
  ...
  async accountGetAuthResult() {
    ...
    try {
      const lastAuthResult = await this.accountAuthManager.getAuthResult(packageName);
      alert('getAuthResult -> success: ' + JSON.stringify(lastAuthResult));
    } catch (ex) {
      alert('getAuthResult -> Error : ' + JSON.stringify(ex));
    }
  ...
  async accountGetAuthResultWithScopes() {
    ...
    try {
      const authResultWithScope = await this.accountAuthManager.getAuthResultWithScope(authScopeList,packageName);
      alert('getAuthResultWithScopes -> success: ' + JSON.stringify(authResultWithScope));
    } catch (ex) {
      alert('getAuthResultWithScopes -> Error : ' + JSON.stringify(ex));
    }
  ...
  async accountAddAuthScopes() {
    ...
    try {
      const authScope = await this.accountAuthManager.addAuthScopes(8888, authScopeList,packageName);
      alert('addAuthScopes -> success: ' + JSON.stringify(authScope));
    } catch (ex) {
      alert('addAuthScopes -> Error : ' + JSON.stringify(ex));
    }
