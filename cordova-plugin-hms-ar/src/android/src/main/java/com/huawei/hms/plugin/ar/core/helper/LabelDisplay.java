...
public class LabelDisplay {
    ...
    private static final BigDecimal LABEL_WIDTH = new BigDecimal("0.3");
    private static final BigDecimal LABEL_HEIGHT = new BigDecimal("0.3");
    ...
    private void drawSortedPlans(ArrayList<ARPlane> sortedPlanes, float[] cameraViews, float[] cameraProjection) {
        ...
        for (ARPlane plane : sortedPlanes) {
            ...
            BigDecimal scaleU = new BigDecimal("1.0");

            // Set the value of the plane angle uv matrix.
            planeAngleUvMatrix[0] = scaleU.divide(LABEL_WIDTH, BigDecimal.ROUND_CEILING).floatValue();
            planeAngleUvMatrix[1] = 0.0f;
            planeAngleUvMatrix[2] = 0.0f;
            BigDecimal scaleV = scaleU.divide(LABEL_HEIGHT, BigDecimal.ROUND_CEILING);
            planeAngleUvMatrix[3] = scaleV.floatValue();
    ...
    private void drawLabel(float[] cameraViews, float[] cameraProjection) {
        ...
        float halfWidth = LABEL_WIDTH.divide(two, BigDecimal.ROUND_CEILING).floatValue();
        float halfHeight = LABEL_HEIGHT.divide(two, BigDecimal.ROUND_CEILING).floatValue();
        float[] vertices = {
