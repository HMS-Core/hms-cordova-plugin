...
3. Install `HMS Ads Prime plugin` to the project.

    ```bash
    ionic cordova plugin add @hmscore/cordova-plugin-hms-adsprime
    ```

4. Install HMS Ads Prime Ionic Native wrappers
...
The obfuscation is done by **ProGuard.** By default, in Cordova and Ionic apps ProGuard is disabled. Even though ProGuard is not available, ProGuard support can be added through 3rd party ProGuard plugins. If ProGuard is enabled in your project, the Huawei Cordova Ads Prime plugin's ProGuard rules need to be added to your project. These rules are as follows:
