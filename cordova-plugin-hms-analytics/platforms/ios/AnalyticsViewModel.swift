...
public class AnalyticsViewModel {
    ...
    /// - Parameters:
    ///   - reportPolicyType: HAReportPolicy list.
    /// - Important:
    ...
    /// - Returns: Void
    func setReportPolicies(_ reportPolicyType: [String:Any]){
        var mReportPoliciyList = [HAReportPolicy]()
        for key in reportPolicyType.keys{
            if key=="onScheduledTimePolicy" {
                guard let  onScheduledTime = reportPolicyType["onScheduledTimePolicy"] as? Int else {
                    log(logMessage: "Incorrect 'onScheduledTimePolicy' paramater.")
                    continue
                }
                mReportPoliciyList.append(HAReportPolicy.onScheduledTime(onScheduledTime))
                ...
                guard let  _ = reportPolicyType["onAppLaunchPolicy"] as? Bool else {
                    log(logMessage: "Incorrect 'onAppLaunchPolicy' paramater.")
                    continue
                }
                mReportPoliciyList.append(HAReportPolicy.onAppLaunch())
                ...
                guard let  _ = reportPolicyType["onMoveBackgroundPolicy"] as? Bool else {
                    log(logMessage: "Incorrect 'onMoveBackgroundPolicy' paramater.")
                    continue
                }
                mReportPoliciyList.append(HAReportPolicy.onMoveBackground())
            }else if key=="onCacheThresholdPolicy" {
                guard let  onCacheThresholdTime = reportPolicyType["onCacheThresholdPolicy"] as? Int else {
                    log(logMessage: "Incorrect 'onCacheThresholdPolicy' paramater.")
                    continue
                }
                mReportPoliciyList.append(HAReportPolicy.onCacheThresholdPolicy(onCacheThresholdTime))
            }
        }
        if mReportPoliciyList.count>0{
            HiAnalytics.setReportPolicies(mReportPoliciyList)
        }
    }
    ...
    func log(logMessage: String, functionName: String = #function) {
        print("ERROR function -> \(functionName): \(logMessage)")
    }
}
