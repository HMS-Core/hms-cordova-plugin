...
export class TtsPage {
  ...
  public async ttsAnalyserStart(textToSpeech) {
    var ttsAnalyserInput = {
      text: textToSpeech,
        mlConfigs: {
            language: HMSMLLanguage.MLTtsConstants.TTS_EN_US,
            person: HMSMLLanguage.MLTtsConstants.TTS_SPEAKER_FEMALE_EN,
            speed: 1.0,
            volume: 1.0,
            synthesizeMode: HMSMLLanguage.MLTtsConstants.TTS_ONLINE_MODE
        },
        queuingMode: 0
    };
    try {
      await HMSMLLanguage.ttsAnalyser(ttsAnalyserInput);
    } catch (ex) {
      alert(ex)
    }
  }
  public async ttsAnalyserDownload() {
    var ttsAnalyserInput = {
      text: "Hello Huawei!",
        mlConfigs: {
            language: HMSMLLanguage.MLTtsConstants.TTS_EN_US,
            person: HMSMLLanguage.MLTtsConstants.TTS_SPEAKER_OFFLINE_EN_US_FEMALE_BEE,
            speed: 1.0,
            volume: 1.0,
            synthesizeMode: HMSMLLanguage.MLTtsConstants.TTS_OFFLINE_MODE

        },
        queuingMode: 0
    };
    try {
      await HMSMLLanguage.ttsOfflineAnalyser(ttsAnalyserInput);
    } catch (ex) {
      alert(ex)
    }
  }
  public async ttsAnalyserPause() {
    try {
      alert(await HMSMLLanguage.ttsAnalyserPause());
    } catch (ex) {
      alert(ex)
    }
  }
  public async ttsAnalyserResume() {
    try {
      alert(await HMSMLLanguage.ttsAnalyserResume());
    } catch (ex) {
      alert(ex)
    }
  }
  public async ttsAnalyserStop() {
    try {
      alert(await HMSMLLanguage.ttsAnalyserStop());
    } catch (ex) {
      alert(ex)
    }
  }
