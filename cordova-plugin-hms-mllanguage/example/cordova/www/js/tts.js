...
async function ttsAnalyserStart(textToSpeech) {
    var ttsReq = {
        text: textToSpeech,
        mlConfigs: {
            language: HMSMLPlugin.MLTtsConstants.TTS_EN_US,
            person: HMSMLPlugin.MLTtsConstants.TTS_SPEAKER_FEMALE_EN,
            speed: 1.0,
            volume: 1.0,
        },
        queuingMode: 0,
    ...
    try {
        await HMSMLLanguage.ttsAnalyser(ttsReq);
    } catch (Ex) {
...
async function ttsOfflineAnalyser() {
    var ttsReq = {
        ...
        mlConfigs: {
            language: HMSMLPlugin.MLTtsConstants.TTS_LAN_FR_FR,
            person: HMSMLPlugin.MLTtsConstants.TTS_SPEAKER_OFFLINE_FR_FR_FEMALE_BEE,
            speed: 1.0,
            volume: 1.0,
            synthesizeMode: HMSMLPlugin.MLTtsConstants.TTS_OFFLINE_MODE
    ...
    try {
        await HMSMLLanguage.ttsOfflineAnalyser(ttsReq);
    } catch (Ex) {
        alert(JSON.stringify(Ex));
    }
}
