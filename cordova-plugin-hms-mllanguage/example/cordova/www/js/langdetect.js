...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("stopLangDetectionService").addEventListener("click", () => stopTranslatorService());
}, false);
...
async function getlocalLangDetection() {
    ...
    try {
        var langDetectResult = await HMSMLLanguage.localLangDetection(localLangDetectionReq);
...
async function getremoteLangDetection(taskModes) {
    ...
    try {
        var langDetectResult = await HMSMLLanguage.remoteLangDetection(remoteLangDetectionReq);
        if (mode == 0) {
...
async function stopTranslatorService() {
    try {
        alert(await HMSMLLanguage.stopLangDetectionService());
    } catch (ex) {
        alert(JSON.stringify(Ex));
    }
}
