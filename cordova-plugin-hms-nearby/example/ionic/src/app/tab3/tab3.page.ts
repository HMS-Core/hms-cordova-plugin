...
export class Tab3Page {
  ...
  addListeners() {
    // on wifi found
    this.hmsNearby.on(
      HMSNearbyEvent.EVENT_WIFI_ON_FOUND,
      (res: ScanEndpointInfo) => {
        console.log("Wifi on found: " + JSON.stringify(res));

        // share config
        this.hmsNearby
          .shareWifiConfig(res.endpointId)
          .then((res2) => {
    ...
    // on fetch auth code
    this.hmsNearby.on(
      HMSNearbyEvent.EVENT_WIFI_ON_FETCH_AUTH_CODE,
      (res: AuthCodeUpdate) => {
    ...
    // on result
    this.hmsNearby.on(
      HMSNearbyEvent.EVENT_WIFI_ON_SHARE_RESULT,
      (res: WifiShareResult) => {
  ...
  startWifiShare() {
    this.hmsNearby
      .startWifiShare(this.currentPolicy)
      .then((res) => {
