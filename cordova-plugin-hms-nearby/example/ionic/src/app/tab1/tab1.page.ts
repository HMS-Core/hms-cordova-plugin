...
export class Tab1Page {
  ...
  addListeners() {
    this.hmsNearby.on(
      HMSNearbyEvent.EVENT_CONNECTION_ON_ESTABLISH,
      (res: ConnectInfo) => {
    ...
    this.hmsNearby.on(
      HMSNearbyEvent.EVENT_CONNECTION_ON_RESULT,
      (res: ConnectResult) => {
    ...
    this.hmsNearby.on(
      HMSNearbyEvent.EVENT_CONNECTION_ON_DISCONNECT,
      (res: EndpointId) => {
    ...
    this.hmsNearby.on(
      HMSNearbyEvent.EVENT_SCAN_ON_FOUND,
      (res: ScanEndpointInfo) => {
    ...
    this.hmsNearby.on(HMSNearbyEvent.EVENT_DATA_ON_RECEIVED, (res: Data) => {
      ...
      if (res.dataType === DataType.DATA_BYTES) {
        ...
        this.pushMessage(
          this.friendName,
          "File is receiving(Nearby/" + res.dataId + ")",
          "start",
          "left"
        );
      }
    ...
    this.hmsNearby.on(
      HMSNearbyEvent.EVENT_DATA_ON_TRANSFER_UPDATE,
      (res: TransferStateUpdate) => {
  ...
  startBroadcasting(name: string, serviceId: string) {
    this.hmsNearby
      .startBroadcasting(name, serviceId, Policy.POLICY_P2P)
      .then(() => {
      ...
      .catch((err) =>
        this.presentToast("Broadcasting start error:" + JSON.stringify(err))
      );
  }
  ...
  stopBroadcasting() {
    this.hmsNearby
      ...
      .catch((err) =>
        this.presentToast("Broadcasting stop error:" + JSON.stringify(err))
      );
  }

  startScan(serviceId: string) {
    this.hmsNearby
      .startScan(serviceId, Policy.POLICY_P2P)
      .then(() => {
      ...
      .catch((err) =>
        this.presentToast("Scan start error:" + JSON.stringify(err))
      );
  }
  ...
  stopScan() {
    this.hmsNearby
      ...
      .catch((err) =>
        this.presentToast("Scan stop error:" + JSON.stringify(err))
      );
  }
  ...
  acceptConnect(endpointId: string) {
    this.hmsNearby
      ...
      .then((res) =>
        this.presentToast("Connection accepted:" + JSON.stringify(res))
      )
      .catch((err) =>
        this.presentToast("Connection accept error:" + JSON.stringify(err))
      );
  }
  ...
  requestConnect(name: string, endpointId: string) {
    ...
    this.hmsNearby
      .requestConnectEx(name, endpointId, channelPolicy)
      .then((res) =>
        this.presentToast("Connection requested:" + JSON.stringify(res))
      )
      .catch((err) =>
        this.presentToast("Connection request error:" + JSON.stringify(err))
      );
  }
  ...
  disconnect(endpointId: string) {
    this.hmsNearby
      ...
      .catch((err) =>
        this.presentToast("Disconnect error:" + JSON.stringify(err))
      );
  }
