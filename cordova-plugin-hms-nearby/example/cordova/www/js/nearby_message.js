...
var nearby_message = {
  ...
  addEventListeners: function () {
    ...
    HMSNearby.on(HMSNearby.HMSNearbyEvent.EVENT_MESSAGE_ON_LOST, (res) => {
      ...
      const index = messageListMessage.findIndex(
        (item) => item.namespace === res.namespace && item.content === content
      );
      if (index === -1) {
        console.log(
          "listener :: EVENT_MESSAGE_ON_LOST index not found for message: " +
            content
        );
        return;
      }
      ...
      this.refreshMessagePane();
      console.log(
        "listener :: EVENT_MESSAGE_ON_LOST message deleted: " + content
      );
    });

    HMSNearby.on(
      HMSNearby.HMSNearbyEvent.EVENT_MESSAGE_ON_BLE_SIGNAL_CHANGED,
      (res) => {
        console.log(
          "listener :: EVENT_MESSAGE_ON_BLE_SIGNAL_CHANGED: " +
            JSON.stringify(res)
        );
      }
    );
    ...
    HMSNearby.on(
      HMSNearby.HMSNearbyEvent.EVENT_MESSAGE_ON_DISTANCE_CHANGED,
      (res) => {
        console.log(
          "listener :: EVENT_MESSAGE_ON_DISTANCE_CHANGED: " +
            JSON.stringify(res)
        );
      }
    );
  ...
  addClickListeners: function () {
    ...
    document.getElementById("btn-get").addEventListener("click", () => {
      ...
      HMSNearby.get(HMSNearby.GET_OPTION_DEFAULT)
        .then((res) => {
