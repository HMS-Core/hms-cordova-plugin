...
@Plugin({
    pluginName: 'HMSSite',
    plugin: 'cordova-plugin-hms-site',
    pluginRef: 'HMSSite',
    repo: '',
    install: '',
    installVariables: [],
...
export class HMSSite extends IonicNativePlugin {
    ...
    @Cordova({ otherPromise: true })
    gotoSearchActivity(apiKey: string, searchFilter: SearchFilter, hint?: string): Promise<Site> {
        return;
    }
}
...
export class SearchServiceFactory {
    static async create(apiKey: string): Promise<SearchService> {
        return await (<any>window).HMSSite.SearchServiceFactory.create(apiKey);
    }
}
