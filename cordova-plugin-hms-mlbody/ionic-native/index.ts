...
@Plugin({
    ...
    plugin: "cordova-plugin-hms-mlbody", // npm package name, example: cordova-plugin-camera
    pluginRef: "HMSMLBodyPlugin", // the variable reference to call the plugin, example: navigator.geolocation
...
@Plugin({
    pluginName: "HMSMLBody",
    plugin: "cordova-plugin-hms-mlbody", // npm package name, example: cordova-plugin-camera
    pluginRef: "HMSMLBody", // the variable reference to call the plugin, example: navigator.geolocation
    platforms: ["Android"], // Array of platforms supported, example: ['Android', 'iOS']
})
@Injectable()
export class HMSMLBody extends IonicNativePlugin {
   ...
   */
    ...
    interactiveLivenessAnalyser(
        interactiveLivenessDetectionReq: interactiveLivenessDetectionReq
    ): Promise<MLInteractiveLivenessCaptureResult> {
        return;
    }
    @Cordova({ otherPromise: true })
    /**
    ...
    customInteractiveLivenessAnalyser(
        customInteractiveLivenessDetectionReq: customInteractiveLivenessDetectionReq
    ): Promise<MLInteractiveLivenessCaptureResult> {
        return;
    }
...
export enum InteractiveLivenessStateCode {
    FACE_ASPECT = 1001,
    NO_FACE = 1002,
    MULTI_FACES = 1003,
    PART_FACE = 1004,
