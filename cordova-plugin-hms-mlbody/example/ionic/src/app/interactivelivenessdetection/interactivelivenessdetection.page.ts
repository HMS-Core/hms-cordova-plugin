...
@Component({
  selector: 'app-interactivelivenessdetection',
  templateUrl: './interactivelivenessdetection.page.html',
  styleUrls: ['./interactivelivenessdetection.page.scss'],
})
export class InteractivelivenessdetectionPage {
  constructor() { 
  ...
  public async getLiveness() {
    try {
      var interactiveLivenessDetectionReq = {
        options: HMSMLBody.Options.DETECT_MASK,
        detectionTimeOut: 10000
      };
      const result = await HMSMLBody.interactiveLivenessAnalyser(interactiveLivenessDetectionReq);
  ...
  public async getCustomLiveness() {
    try {
      var statusCodes = {
      ...
      const result = await HMSMLBody.customInteractiveLivenessAnalyser(interactiveLivenessDetectionReq);
      alert("Success: " + JSON.stringify(result));
    } catch (ex) {
      alert("Error: " + JSON.stringify(ex))
    }
