...
export class ClassifyPage {
  ...
  public async changeToogle() {
    ...
    if (this.toggle == true) {
      this.ocrType = HMSMLImage.MLImageClassificationConfig.TYPE_LOCAL;
    } else {
      this.ocrType = HMSMLImage.MLImageClassificationConfig.TYPE_REMOTE;
    }
  }
  ...
  public async imageClassificationAnalyser() {
    ...
    try {
      ...
      if (this.toggle == true) {
        var result = await HMSMLImage.imageClassificationAnalyser(remoteImageClassificationInput);
        alert(JSON.stringify(result));
      } else {
        var result = await HMSMLImage.imageClassificationAnalyser(localImageClassificationInput);
        alert(JSON.stringify(result));
      }
