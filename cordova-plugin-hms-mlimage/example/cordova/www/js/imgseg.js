...
async function getimgSegmentation() {
    var imgSegmentationReq = {
        imageSegmentationSetting: {
            analyserType: HMSMLImagePlugin.MLImageSegmentationSetting.BODY_SEG,
            scene: HMSMLImagePlugin.MLImageSegmentationScene.ALL,
            analyseMode: 1
        },
        filePath: $imageResult,
    };
    try {
        const result = await HMSMLImage.imgSegmentation(imgSegmentationReq)
