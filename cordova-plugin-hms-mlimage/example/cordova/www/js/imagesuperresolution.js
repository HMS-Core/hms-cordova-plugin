...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("getSuperResolutionImagePicker").addEventListener("click", function () { getimgPicker(); });
}, false);
...
async function imageSuperResolution() {
    ...
    try {
        let result = await HMSMLImage.imageSuperResolution(imageSuperResolutionReq);
...
async function imageSuperResolution3X() {
    ...
    try {
        let result = await HMSMLImage.imageSuperResolution(imageSuperResolutionInput);
        document.getElementById("superResolutionImage").src = "data:image/jpeg;base64," + result.bitmap;
    } catch (Ex) {
        alert(JSON.stringify(Ex));
    }
