...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("getImageClassificationPicker").addEventListener("click", function () { getimgPicker(); });
}, false);
...
async function imageClassificationAnalyser() {
    ...
    try {
        var result = await HMSMLImage.imageClassificationAnalyser(imageClassificationReq);
...
async function stopImageClassificationAnalyser() {
    try {
        alert(await HMSMLImage.stopImageClassificationAnalyser())
    } catch (Ex) {
        alert(JSON.stringify(Ex));
    }
}
