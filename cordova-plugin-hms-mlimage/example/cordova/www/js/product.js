...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("getproductPicker").addEventListener("click", function () { getimgPicker(); });
}, false);
...
async function productAnalyser() {
    ...
    try {
        if (device == true) {
            const result = await HMSMLImage.productVisionAnalyser(productReq);
            alert(JSON.stringify(result));
           
        } else {
            const result = await HMSMLImage.productVisionAnalyser(productPluginReq);
            alert(JSON.stringify(result));
        }
    } catch (Ex) {
