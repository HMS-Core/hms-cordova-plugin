...
export class HomePage {
  ...
  obtainProductInfoFromType = async (pType) => {
    try {
      ...
      console.log(message);
      message.productInfoList.map((p) =>
        this.createAvailableProductOnList(p, pType)
      );
    } catch (err) {
      console.log(err);
    }
  ...
  obtainOwnedPurchasesFromType = async (pType) => {
    try {
      ...
      message.itemList.map((pId, ind) =>
        this.createPurchasedProductOnList(
          pId,
          message.inAppPurchaseDataList[ind],
          pType
        )
      );
    } catch (err) {
  ...
  obtainOwnedPurchaseRecordFromType = async (pType) => {
    try {
      ...
      console.log(message);
      message.itemList.map((pId) =>
        this.createPurchasedRecordProductOnList(pId, pType)
      );
    } catch (err) {
      console.log(err);
    }
  ...
  getProduct = (productId, productType) => {
    const pList = PRODUCTS[productType].products.filter(
      (p) => p.id === productId
    );
